	 ...
	 QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		while timeLeft > 0 do
			...
			if #(GetEntityCoords(playerPed)-Config.CircleZones.MethProcessing.coords) > 2 then
				exports.qbx_core:Notify(Lang:t("error.too_far"), "error")
		...
		if not isProcessing then
			QBCore.Functions.TriggerCallback('ps-drugprocessing:validate_items', function(result)
				if result.ret then
					...
					exports.qbx_core:Notify(Lang:t("error.no_item", {item = result.item}))
				end
			...
			exports.qbx_core:Notify(Lang:t("error.already_processing"), 'error')
		end
	end
end)
...
RegisterNetEvent('ps-drugprocessing:ChangeTemp', function()
	if not isTempChangeU then
		QBCore.Functions.TriggerCallback('ps-drugprocessing:validate_items', function(result)
			if result.ret then
				local success = lib.skillCheck({'easy', 'easy', {areaSize = 60, speedMultiplier = 1}, 'easy'}, {'w', 'a', 's', 'd'})
					if success then
						exports.qbx_core:Notify(Lang:t("success.temp_up"), 'success')
						ProcessTempUp()
					else
						TriggerServerEvent('ps-drugprocessing:cancelProcessing')
		...
		exports.qbx_core:Notify(Lang:t("error.enough_temp"), 'error')
	end
end)
	...
	if not isTempChangeD then
		QBCore.Functions.TriggerCallback('ps-drugprocessing:validate_items', function(result)
			if result.ret then
				local success = lib.skillCheck({'easy', 'easy', {areaSize = 60, speedMultiplier = 1}, 'easy'}, {'w', 'a', 's', 'd'})
					if success then
						exports.qbx_core:Notify(Lang:t("success.temp_down"), 'success')
						ProcessTempDown()
					else
						TriggerServerEvent('ps-drugprocessing:cancelProcessing')
